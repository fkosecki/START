...
public class TodoServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        resp.setContentType("application/json;char=UTF-8");
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        try {
            var newTodo = repository.addTodo(mapper.readValue(req.getInputStream(), TODO.class));
            resp.setContentType("application/json;char=UTF-8");
            mapper.writeValue(resp.getOutputStream(), newTodo);
        }
        catch (Exception e){
            logger.warn("Wrong object inserted: " + req.getInputStream().toString());
        }
    }
}
